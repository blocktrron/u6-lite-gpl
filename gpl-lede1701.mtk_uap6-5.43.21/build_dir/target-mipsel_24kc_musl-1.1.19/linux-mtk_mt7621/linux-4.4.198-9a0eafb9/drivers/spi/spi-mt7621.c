...
void
mt7621_spi_lock(void)
{
	if (!g_master) {
		printk("mt7621_spi: no bus to lock\n");
		return;
	}
	mutex_lock(&g_master->bus_lock_mutex);
}
EXPORT_SYMBOL(mt7621_spi_lock);

void
mt7621_spi_unlock(void)
{
	if (!g_master) {
		printk("mt7621_spi: no bus to unlock\n");
		return;
	}
	mutex_unlock(&g_master->bus_lock_mutex);
}
EXPORT_SYMBOL(mt7621_spi_unlock);
