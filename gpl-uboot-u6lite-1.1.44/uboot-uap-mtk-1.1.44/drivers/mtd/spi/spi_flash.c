...
{
	u32 erase_size_max, erase_size_min, erase_size, erase_addr;
	u8 cmd[SPI_FLASH_CMD_LEN];
	int ret = -1;

	erase_size_max = flash->erase_max_size;
	erase_size_min = flash->erase_min_size;
	if (offset % erase_size_min || len % erase_size_min) {
	...
	while (len) {
		...
		if ((erase_addr % erase_size_max == 0) && (len >= erase_size_max)) {
			cmd[0] = flash->erase_max_cmd;
			erase_size = erase_size_max;
		}
		else {
			cmd[0] = flash->erase_min_cmd;
			erase_size = erase_size_min;
		}
...
int stm_unlock(struct spi_flash *flash, u32 ofs, size_t len)
{
	...
	if (stm_is_locked_sr(flash, ofs - flash->erase_max_size, flash->erase_max_size,
			     status_old))
		return -EINVAL;
	/*
...
int spi_flash_scan(struct spi_flash *flash)
{
	...
	flash->erase_max_cmd = CMD_ERASE_64K;
	flash->erase_max_size = flash->sector_size;

	/* Compute erase sector and command */
	if (info->flags & SECT_4K) {
		flash->erase_min_cmd = CMD_ERASE_4K;
		flash->erase_min_size = 4096 << flash->shift;
		flash->sector_size = flash->erase_min_size;
	}
	else {
		flash->erase_min_cmd = flash->erase_max_cmd;
		flash->erase_min_size = flash->erase_max_size;
	}

	/* Always uses the minimum erase size */
	flash->erase_size = flash->erase_min_size;
	...
	print_size(flash->erase_min_size, ", total ");
