...
extern int create_mt7621_nand_bbt_layer(struct mtd_info *bott_mtd, struct mtd_info **upper_mtd);
...
static inline u8 *page_data_ptr(struct nand_chip *chip, const u8 *buf, int sector_index)
{
...
static int nfc_wait_write_completion(mt7621_nfc_t *nfc, struct nand_chip *chip)
{
	...
	if (ret)
		printf("Error: NFI master timed out waiting for write completion\n");
...
static void nfc_cmd_ctrl(struct mtd_info *mtd, int dat, unsigned int ctrl)
{
	...
	if (ctrl & NAND_ALE)
	{
		nfc_issue_address(nfc, dat & 0xff, 0, 1, 0);
	}
	else if (ctrl & NAND_CLE)
	{
		nfc_hw_reset(nfc);
		nfi_write16(nfc, NFI_CNFG_REG16, REG_SET_VAL(OP_MODE, OP_CUSTOM));
		nfc_issue_cmd(nfc, dat);
	}
}
...
static void nfc_pio_write(struct mtd_info *mtd, u32 val, int bw)
{
	...
	{
		if (bw)
			nfi_clrsetbits16(nfc, NFI_CNFG_REG16, READ_MODE, BYTE_RW);
		else
			nfi_clrbits16(nfc, NFI_CNFG_REG16, READ_MODE | BYTE_RW);
...
static u32 nfc_pio_read(struct mtd_info *mtd, int br)
{
	...
	{
		if (br)
			nfi_setbits16(nfc, NFI_CNFG_REG16, READ_MODE | BYTE_RW);
		else
			nfi_clrsetbits16(nfc, NFI_CNFG_REG16, BYTE_RW, READ_MODE);
...
#ifdef CONFIG_NAND_MT7621_LEGACY_OOB_LAYOUT
...
static void nfc_write_oob(struct nand_chip *chip, void *fdm)
{
	...
	if ((size_t) fdm % sizeof(u32) == 0)
	{
		for (i = 0; i < chip->ecc.steps; i++)
		{
			nfi_write32(nfc, NFI_FDML_REG32(i), *p++);
			nfi_write32(nfc, NFI_FDMM_REG32(i), *p++);
		}
	}
	else
	{
		...
		{
			memcpy(&tmp, p++, 4);
			nfi_write32(nfc, NFI_FDML_REG32(i), tmp);
			memcpy(&tmp, p++, 4);
			nfi_write32(nfc, NFI_FDMM_REG32(i), tmp);
		}
	}
}
#endif
	...
	u8 *buf, int oob_on, int page)
{
	...
	nfi_setbits16(nfc, NFI_CNFG_REG16, READ_MODE | AUTO_FMT_EN | HW_ECC_EN);

	nfc_ecc_init(nfc, &chip->ecc);
	nfc_ecc_decoder_start(nfc);

	nfi_write16(nfc, NFI_CON_REG16, NFI_BRD | REG_SET_VAL(NFI_SEC, chip->ecc.steps));
	...
	for (i = 0; i < chip->ecc.steps; i++)
	{
		...
		{
			rc = nfc_ecc_correct_check(nfc, nfc_sel, page_data_ptr(chip, buf, i), i);
			if (rc > 0 && bitflips >= 0)
				bitflips += rc;
		}
	}

	nfc_read_oob(chip, chip->oob_poi);
	...
	u8 *buf, int oob_on, int page)
{
	...
	nfc_read_buf(mtd, nfc_sel->page_cache, mtd->writesize + mtd->oobsize);
	...
	{
		for (i = 0; i < chip->ecc.steps; i++)
			memcpy(page_data_ptr(chip, buf, i), raw_page_data_ptr(nfc_sel, i), chip->ecc.size);
	}
...
static int nfc_check_empty_page(struct mtd_info *mtd, struct nand_chip *chip, const u8 *buf)
{
	uint32_t i, j;
	u8 *oob_ptr;
	...
	const u8 *buf, int oob_on, int page)
{
	...
	nfi_clrsetbits16(nfc, NFI_CNFG_REG16, READ_MODE, AUTO_FMT_EN | HW_ECC_EN);
	...
	nfi_write16(nfc, NFI_CON_REG16, NFI_BWR | REG_SET_VAL(NFI_SEC, chip->ecc.steps));
	...
	const u8 *buf, int oob_on, int page)
{
	...
	{
		for (i = 0; i < chip->ecc.steps; i++)
			memcpy(raw_page_data_ptr(nfc_sel, i), page_data_ptr(chip, buf, i), chip->ecc.size);
	}

	nfi_clrbits16(nfc, NFI_CNFG_REG16, READ_MODE | AUTO_FMT_EN | HW_ECC_EN);

	nfc_write_buf(mtd, nfc_sel->page_cache, mtd->writesize + mtd->oobsize);
...
static int nfc_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	/* XXX: For BootROM compatibility, always write to offset 0 */
	chip->oob_poi[0] = 0;
...
static void nfc_set_nand_ecc_layout(mt7621_nfc_sel_t *nfc_sel)
{
	...
	fdm_size = chip->ecc.steps * NFI_ECC_FDM_SIZE;
	...
	nfc_sel->ecc_layout.oobfree[0].offset = 1;
	nfc_sel->ecc_layout.oobfree[0].length = fdm_size - 1;
}
	...
	struct mtd_oob_region *oob_region)
{
	...
	oob_region->offset = 1;
	oob_region->length = chip->ecc.steps * NFI_ECC_FDM_SIZE - 1;
...
static void nfc_probe(mt7621_nfc_t *nfc, int cs)
{
	...
	nfc_sel->page_cache = malloc(chip->mtd.writesize + chip->mtd.oobsize + ARCH_DMA_MINALIGN - 1);
	if (!nfc_sel->page_cache)
		return;

	nfc_sel->page_cache = (char *) ALIGN((u32) nfc_sel->page_cache, ARCH_DMA_MINALIGN);

	nfc_sel->oob_mb_cache = malloc(chip->mtd.oobsize + ARCH_DMA_MINALIGN - 1);
	if (!nfc_sel->oob_mb_cache)
		return;

	nfc_sel->oob_mb_cache = (char *) ALIGN((u32) nfc_sel->oob_mb_cache, ARCH_DMA_MINALIGN);
	...
	if (is_power_of_2(chip->mtd.erasesize))
		chip->mtd.erasesize_shift = ffs(chip->mtd.erasesize) - 1;
	else
		chip->mtd.erasesize_shift = 0;

	if (is_power_of_2(chip->mtd.writesize))
		chip->mtd.writesize_shift = ffs(chip->mtd.writesize) - 1;
	else
		chip->mtd.writesize_shift = 0;

	chip->mtd.erasesize_mask = (1 << chip->mtd.erasesize_shift) - 1;
	chip->mtd.writesize_mask = (1 << chip->mtd.writesize_shift) - 1;

	if (create_mt7621_nand_bbt_layer(&chip->mtd, &nfc_sel->upper_mtd) == 0)
		nand_register(cs, nfc_sel->upper_mtd);
#else
